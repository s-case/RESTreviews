...
public class PutaccountHandler{
    ...
    public PutaccountHandler(String authHeader, int accountId, JavaaccountModel oJavaaccountModel, UriInfo oApplicationUri){
    ...
    public JavaaccountModel createHypermedia(JavaaccountModel oJavaaccountModel){
        ...
        oJavaaccountModel.getlinklist().add(new HypermediaLink(String.format("%s%s/%s", oApplicationUri.getBaseUri(), oRelativePath, "purchase"), "Get all the purchases of this account", "GET", "Child"));
        oJavaaccountModel.getlinklist().add(new HypermediaLink(String.format("%s%s/%s", oApplicationUri.getBaseUri(), oRelativePath, "purchase"), "Create a new purchase for this account", "POST", "Child"));
        ...
        int iLastSlashIndex = String.format("%s%s", oApplicationUri.getBaseUri(), String.format("%s", oApplicationUri.getPath()).replaceAll("account","accountManager")).lastIndexOf("/");
        oJavaaccountModel.getlinklist().add(new HypermediaLink(String.format("%s%s", oApplicationUri.getBaseUri(), String.format("%s", oApplicationUri.getPath()).replaceAll("account","accountManager")).substring(0, iLastSlashIndex), "Create a new account", "POST", "Parent"));
        oJavaaccountModel.getlinklist().add(new HypermediaLink(String.format("%s%s", oApplicationUri.getBaseUri(), String.format("%s", oApplicationUri.getPath()).replaceAll("account","accountManager")).substring(0, iLastSlashIndex), "Get all accounts", "GET", "Parent"));
